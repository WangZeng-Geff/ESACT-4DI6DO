...
static bool is_key_press(u8 idx)
{
    if(idx >= RESTORE_KEY_NUM) return false;
	
		uint8_t key_no = idx; //< dev_state.key_num ? dev_state.key_reverse[idx] : idx;
	
		if (key_no > sizeof(keys)) return false;

    return GPIO_ReadBit(keys[key_no].port, keys[key_no].pin) == SET;
}
