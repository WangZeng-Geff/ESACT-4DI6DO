...
static void buz_gpio_init(void)
{
    ...
    T16Nx_MAT0Out0Config(T16N3,TIM_Out_High);           //匹配2：输出高
    T16Nx_MAT1Out0Config(T16N3,TIM_Out_Low);            //匹配3：输出低

    T16Nx_SetPREMAT(T16N3,configSYS_CLOCK / 2000000 - 1);   //预分频1:8,，8MHz->500kHz
    T16Nx_SetMAT0(T16N3,1000);                          //控制占空比
    T16Nx_SetMAT1(T16N3,2000);                          //控制频率

    NVIC_Init(NVIC_T16N3_IRQn,NVIC_Priority_1,DISABLE);

    T16Nx_MAT0ITConfig(T16N3,TIM_Go_No);               //匹配2：继续计数
    T16Nx_MAT1ITConfig(T16N3,TIM_Clr_Int);              //匹配3：清0
