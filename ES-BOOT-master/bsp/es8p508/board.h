...
#define board_get_update_para(para, len)   \
     es8p508_flash_read(UPDATE_PARA_ADDR, (para), (len))
#define board_get_update_data(off, data, len) \
     es8p508_flash_read(UPDATE_DATA_ADDR+(off), (data), (len))
#define board_erase_update_para(off, len)   \
     es8p508_flash_erase(UPDATE_PARA_ADDR+(off), (len))

#define board_erase_app(size)   \
     es8p508_flash_erase(APP_ADDR, size)
#define board_read_app(off, data, len)	\
     es8p508_flash_read(APP_ADDR+(off), (data), (len))
#define board_write_app(off, data, len)	\
     es8p508_flash_write(APP_ADDR+(off), (data), (len))
